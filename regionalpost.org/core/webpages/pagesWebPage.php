...
class pagesWebPage extends MasterWebPage {
    ...
    private function InitPage() {
        ...
        // Custom Image (intsc) = 1-based index into the article gallery used ONLY for social previews.
        $ogImagePath = $this->resolveSocialShareImage($page, $defaultImage);
    ...
    /**
     * Prefer admin "Custom Image" gallery slot for social OG/Telegram/Facebook.
     * intsc is 1-based index into pages-galery. Falls back to cover image.
     */
    private function resolveSocialShareImage($page, $defaultImage) {
        $index = intval($page->getProp('intsc')) - 1;
        if ($index >= 0) {
            $files = $page->getFiles('pages-galery');
            if (isset($files[$index]) && is_array($files[$index])) {
                // Prefer full file over thumb for share cards.
                if (!empty($files[$index]['filename'])) {
                    return $files[$index]['filename'];
                }
                if (!empty($files[$index]['thumbname'])) {
                    return $files[$index]['thumbname'];
                }
            }
        }
