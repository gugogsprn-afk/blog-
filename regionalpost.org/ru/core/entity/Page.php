...
class Page extends SiteEntity {
    ...
    public function __construct($mId) {
        ...
        $this->FieldArray =  Array('tp.id', 'tp.name_#','tp.descr_#','tp.content_#','tp.visible','tp.block_id','tf.thumbname','tf.filename',"CONCAT('".HttpContext::current()->culture()->LanguagePath()."',tu.url) as url","CASE WHEN tu.meta_robots=0 THEN 'true' ELSE '' END as mfol",'tp.date_object_$','tp.parent_id','tp.tags','tp.tagsb','tp.alias','tp.ainfo_#','tp.binfo_#',
            ...
            'tp.price','tp.intsa','tp.intsb','tp.intsc');
    ...
    public static function loadFixed($alias) {
        ...
        try {
            ...
            // Missing fixed pages must not take down the whole site.
            $page->ID = 0;
        }
        return $page;
    }
    ...
    public function getFiles($filetype) {
        ...
        $DB->Query("SELECT id, parent_id, type, filename, thumbname, isdefault, img_width, img_height, tmb_width, tmb_height, mime_type, mime_thumb, descr_# as descr, tag, pos FROM #__files WHERE type=".$DB->EscapeValue($filetype)." AND parent_id=".$iid);
