...
$(document).ready(function () {
    ...
    (function () {
        var $switch = $('.rp-lang-switch');
        if (!$switch.length) return;
        $switch.on('click', '.rp-lang-current', function (e) {
            e.preventDefault();
            e.stopPropagation();
            $switch.toggleClass('is-open');
            $(this).attr('aria-expanded', $switch.hasClass('is-open') ? 'true' : 'false');
        });
        $(document).on('click', function () {
            $switch.removeClass('is-open');
            $switch.find('.rp-lang-current').attr('aria-expanded', 'false');
        });
    })();

    (function () {
        var params = new URLSearchParams(window.location.search);
        var show = params.get('show') || 'pages';
        var block = params.get('block_id') || '';
        var key = show + (block ? ':' + block : '');
        $('.rp-subnav-link').each(function () {
            var navKey = $(this).data('rpNav');
            if (navKey === key) {
