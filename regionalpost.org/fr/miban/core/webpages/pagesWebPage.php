<?php
    ...
    private $AllowAdd = array('menu','other','imageitems','publicat','specitems', 'works', 'flatitems');
    private $AllowMove = array('menu','other','imageitems','publicat', 'specitems', 'works', 'flatitems');
    ...
    private $imageSettings = array(
        'flatitems'=>array(
            'createthumb'=>false,
            'thumb'=>array('mode'=>'smarty','w'=>1890,'h'=>740),
            'image'=>array('mode'=>'smarty','w'=>1440,'h'=>970),
            'ext'=>'jpg'
        ),
        'imageitems'=>array(
            'createthumb'=>false,
        ...
        'publicat'=>array(
            'createthumb'=>false,
            'thumb'=>array('mode'=>'smarty','w'=>550,'h'=>735),
            'image'=>array('mode'=>'smarty','w'=>550,'h'=>735),
            'file'=>true,
            'ext'=>'jpg'
        ),
        'specitems'=>array(
    ...
    public function Form() {
        ...
        if ($block_ID==='imageitems') {
            ...
            $tpl->vars['cats'] = SerilizeToJson($catsTpl->GetAllWithChildren());
    ...
    private function PageAdded($page) {
        ...
        if (array_key_exists($page->getProp('block_id'), $this->imageSettings)) {
            ...
            if (($imageSettings['thumb']===true || $imageSettings['image']===true || !empty($imageSettings['file'])) && isset($_FILES['file']['name']) && !empty($_FILES['file']['name'])) {
                if ($file===null) {
                    $file = ItemFile::CreateEmpty($page->ID, 'pages');
                }
