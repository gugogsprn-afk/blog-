...
class MasterWebPage extends WebPageBase {
    ...
    public function Index() {
        // Empty dashboard looked broken — land on Articles list.
        header('Location: index.php?show=pages&block_id=imageitems');
        exit;
    }
    ...
    /**
     * Detect which admin install we are in and expose EN/RU/FR switcher links.
     * Article CRUD stays the same — this only drives the shell chrome.
     */
    protected function assignAdminLangSwitcher($tpl) {
        ...
        // Keep the same screen when hopping between language admins.
        if ($qs === '' || stripos($qs, 'logout=') !== false) {
            $qs = 'show=pages&block_id=imageitems';
        }
        // Never reuse a previous SSO token in the switcher links.
        parse_str($qs, $qsParts);
        unset($qsParts['admin_sso']);
        ...
        $urlEn = $origin . '/miban/index.php' . $suffix;
        $urlRu = $origin . '/ru/miban/index.php' . $suffix;
        $urlFr = $origin . '/fr/miban/index.php' . $suffix;

        // If logged in, attach one-time SSO token so the other panel opens without re-login.
        $identity = HttpContext::current()->Identity();
        if ($identity && HttpContext::current()->Identified() && !empty($identity->username)) {
            $urlEn = AdminSso::appendTokenToUrl($urlEn, $identity->username);
            $urlRu = AdminSso::appendTokenToUrl($urlRu, $identity->username);
            $urlFr = AdminSso::appendTokenToUrl($urlFr, $identity->username);
        }
        ...
        $tpl->vars['admin_url_en'] = $urlEn;
        $tpl->vars['admin_url_ru'] = $urlRu;
        $tpl->vars['admin_url_fr'] = $urlFr;
    }
